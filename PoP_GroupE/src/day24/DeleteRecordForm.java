...
public class DeleteRecordForm extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		btn2.setOnAction(event->{
			//Delete
			new StudentCRUDImplement().delete(Integer.parseInt(txt1.getText()));
			clearForm();
		});
		...
		btn4.relocate(220, 340);
		btn4.relocate(275, 20); //set location
		
		btn4.setOnAction(event->{
			//Search and display record
			int tmpSid = Integer.parseInt(txt1.getText());
			Student student = new StudentCRUDImplement().search(tmpSid);
			if(student==null) {
			...
			txt2.setText(student.getFullName());
			txt3.setText(Double.toString(student.getPop()));
			txt4.setText(Double.toString(student.getCn()));
			txt5.setText(Double.toString(student.getDb()));
			txt6.setText(Double.toString(student.getTotal()));
			txt7.setText(Double.toString(student.getAverage()));
			txt8.setText(student.getResult());
		});
		...
		primaryStage.setTitle("Edit Record Form");
		
		pane.getChildren().addAll(lbl1, txt1, btn4);
